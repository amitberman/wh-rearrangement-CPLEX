...
echo "dataset,scenario,warm_time_sec,cold_time_sec,cold_over_warm,warm_basis_loads,warm_diff_updates,warm_rebuilds,warm_rebuild_ratio,cold_diff_updates,cold_rebuilds" > "$OUT_CSV"
...
for scen in "${SCEN_FILES[@]}"; do
    ...
    echo "$dataset,$(basename "$scen"),$warm_time,$cold_time,$speedup,$warm_basis_loads,$warm_diff_updates,$warm_rebuilds,$warm_rebuild_ratio,$cold_diff_updates,$cold_rebuilds" >> "$OUT_CSV"
done

echo "Warm-start benchmark complete: $OUT_CSV"