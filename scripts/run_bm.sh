...
for bin in "$ORTOOLS_BIN" "$CPLEX_BIN"; do
    if [[ ! -x "$bin" ]]; then
...
for scen in "${SCEN_FILES[@]}"; do
    ...
    IFS='|' read -r cplex_cost cplex_makespan cplex_time _ <<< "$cplex_res"

    [[ -z "$or_cost" ]] && or_cost="-"
    [[ -z "$or_makespan" ]] && or_makespan="-"
    [[ -z "$or_time" ]] && or_time="-"

    [[ -z "$cplex_cost" ]] && cplex_cost="-"
    [[ -z "$cplex_makespan" ]] && cplex_makespan="-"
    [[ -z "$cplex_time" ]] && cplex_time="-"

    cost_match="$(match_status "$or_cost" "$cplex_cost")"
    makespan_match="$(match_status "$or_makespan" "$cplex_makespan")"
    speedup_or_cplex="$(calc_ratio "$or_time" "$cplex_time")"

    echo "| \`$dataset\` | \`$(basename "$scen")\` | $cost_match | $makespan_match | $or_time | $cplex_time | $speedup_or_cplex |" >> "$OUT_MD"
done
